...
import com.king.functions.excel.Reflection.DynamicCreateObject;
import com.king.functions.excel.Utils.CreateFileUtil;
import com.king.functions.excel.Utils.ExcelUtil;
...
//    private DynamicCreateObject dynamicCreateObject;

    public BaseExportExcel() {
    }

//    public BaseExportExcel(DynamicCreateObject dynamicCreateObject) {
//        this.dynamicCreateObject = dynamicCreateObject;
//    }

    // 构造函数，传入要导出的数据
    public BaseExportExcel(String title, String[] rowName, List<Object[]> dataList) {
    ...
    // 导出数据
    private void export(OutputStream out, boolean header) throws Exception {
        try {
    ...
    /***
     * @description: 设置列头
     * @param: [sheet, columnTopStyle, columnNameIndex, columnNum]
     */
...
//        style.setVerticalAlignment(HSSFCellStyle.VERTICAL_CENTER);
    ...
    public void exportExcelFile(String filePath, OutputStream outputStream, boolean header) throws Exception {
        if (ObjectUtils.isNullStringObj(filePath)) {
            filePath = PathUtil.DEFAULT_EXPORT_BASE_PATH + "\\" + excelUtil.randomFileName() + PathUtil.EXCELlSuffix;
        }
